        ...
        <div className="col-span-3 lg:col-span-1 flex-1 w-full max-w-3xl flex order-last lg:order-0 relative">
          <div className="flex w-full bg-white rounded-md shadow-sm border border-transparent focus-within:border-brand-primary-400 transition-colors">
            ...
            <div className="relative flex shrink-0">
              <button
                className="flex items-center justify-between bg-gray-50 px-2 sm:px-4 py-2.5 text-xs sm:text-base text-gray-600 border-r border-gray-200 hover:bg-gray-100 transition-colors whitespace-nowrap rounded-l-md focus:outline-none"
                onClick={() => setIsCategoryDropdownOpen(!isCategoryDropdownOpen)}
              >
                <span className="hidden sm:inline">All Categories</span>
                <span className="sm:hidden">Category</span>
                <ChevronDown className={`w-3 h-3 sm:w-4 sm:h-4 ml-1 sm:ml-2 text-gray-400 transition-transform duration-200 ${isCategoryDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
